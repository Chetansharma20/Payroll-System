...
const Attendences = () => {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        const response = await axios.post(API_ENDPOINTS.ATTENDANCE.FETCH_BY_EMPLOYEE, {
 ...
 const columns = [
  ...
  },
  {
    ...
    flex: 1,
    renderCell: (params) =>
      params.value ? dayjs(params.value).format('HH:mm') : 'N/A'
  },
  {
    field: 'OutPunchTime',
  ...
  margin="normal"
  type="time"
  value={inPunch ? dayjs(inPunch).format('HH:mm') : ''}
...
<TextField
  ...
  margin="normal"
  type="time"
  value={outPunch ? dayjs(outPunch).format('HH:mm') : ''}
