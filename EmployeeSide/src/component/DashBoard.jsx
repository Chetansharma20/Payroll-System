...
const OverView = () => {
  ...
  useEffect(() => {
    const fetchDashboardCounts = async () => {
      try {
        // const EmployeeCount = await axios.post("http://localhost:5000/api/getdashboardcounts",{ EmployeeID:EmployeeData._id});
        const LeaveCount = await axios.post("http://localhost:5000/api/getleavescountbyemployee",{ EmployeeID:EmployeeData._id});
          const AttendanceCount = await axios.post("http://localhost:5000/api/getattendancecountbyemployee",{  EmployeeID:EmployeeData._id});
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 5, marginTop: '10px' }}>
        ...
        Welcome, {EmployeeData.EmployeeName || "Admin"}
      </Typography>
             <Box
               sx={{
                 display: 'flex',
